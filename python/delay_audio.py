...
def delay_audio_with_effects(input_audio_path, index, output_path):
    # 参数设置
    ...
    sf.write(os.path.join(output_dir, 'nodelay.wav'), input_audio, sr)
    sf.write(os.path.join(output_dir, 'delay.wav'), delay_only_audio, sr)
...
if __name__ == '__main__':
    input_folder_path = "D:\\projects\\python\\Audio_Tools\\python\\opencpop_merged"
    output_path = ".\\dataset"
    index = 100
    for _, file_path in enumerate(process_folder(input_folder_path)):
        print('Processing ', index, file_path)
        delay_audio_with_effects(file_path, index, output_path)
        index += 1